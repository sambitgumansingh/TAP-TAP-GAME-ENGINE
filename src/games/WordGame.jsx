...
function WordGame({ config, onBack, onGameEnd }) {
  ...
  function handleSubmit() {
    ...
    const isCorrect = input.trim().toUpperCase() === currentLevel.answer.toUpperCase()
    if (isCorrect) {
      setScore((prev) => prev + config.scoring.correct)
      setMessage("✅ Correct!")
    } else {
      setScore((prev) => prev + config.scoring.wrong)
    ...
    setTimeout(() => {
      ...
      if (levelIndex + 1 < config.levels.length) {
        ...
        setGameOver(true)
        onGameEnd(playerName, score, config.title)
      }
    }, 1500)
  }
  ...
  if (gameOver) {
    return (
      <div className="min-h-screen bg-gray-950 text-white flex flex-col items-center justify-center gap-6">
        ...
        <p className="text-2xl">Final Score: <span className="text-yellow-400 font-bold">{score}</span></p>
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col items-center justify-center gap-8">
      ...
      <div className="flex gap-8 items-center">
        <Timer key={timerKey} timeLimit={config.timeLimit} onTimeUp={() => { setGameOver(true); onGameEnd(playerName, score, config.title) }} />
        <ScoreBoard score={score} total={total} />
      </div>
      <div className="bg-gray-800 rounded-2xl p-8 text-center border border-gray-700 w-80">
        <p className="text-gray-400 mb-2">Level {levelIndex + 1} of {config.levels.length}</p>
        <p className="text-gray-400 text-sm mb-4">Hint: {currentLevel.hint}</p>
        <p className="text-5xl font-bold tracking-widest text-yellow-400 mb-6">{currentLevel.scrambled}</p>
        <input
          type="text"
          value={input}
        ...
        />
        <button onClick={handleSubmit} className="w-full bg-purple-600 hover:bg-purple-500 py-3 rounded-xl font-bold text-lg transition-all">
          Submit
        </button>
